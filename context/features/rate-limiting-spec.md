...
Implement rate limiting on authentication endpointsto prevent brute force attacks, credential stuffing, and abuse of email-sending endpoints.
...
- Display user-firendly error messages on the frontend
...
- Extract IP from `x-forward-for` header (Vercel) or request
- Combine IP from + identifier (email) where applicable for tighter limits
...
- API return 429 status with JSON: `{ error: "Too many attempts. Please try again in X minutes." }`
- Frontend displays error via toast notification
- Include 1Retry-After` header in 429 responses
